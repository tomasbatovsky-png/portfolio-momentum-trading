"""
================================================================================
🎯 PORTFOLIO BACKTEST - PÔVODNÉ DISCOVERY STRATÉGIE
================================================================================

│ Market │ TF │ Filter     │ PF   │ WR   │ Trades/mo │
──────────────────────────────────────────────────────────
│ US100  │ M5 │ ATR 15-80  │ 3.26 │ 66%  │ 1.5       │
│ SP500  │ M5 │ RSI < 70   │ 3.21 │ 69%  │ 0.7       │
│ US30   │ M5 │ RSI < 70   │ 3.29 │ 66%  │ 1.4       │
│ DAX40  │ M5 │ RSI < 70   │ 2.52 │ 62%  │ 1.5       │
│ US30   │ M1 │ M5 trend   │ 2.33 │ 60%  │ 1.4       │
──────────────────────────────────────────────────────────
│ TOTAL  │    │            │ ~3.0 │ 65%  │ ~6.5      │

Parametre pre všetky M5: LB=20, T=0.5, SL=1.5×ATR
US30 M1: LB=10, T=0.7, SL=1.5×ATR
...
# Based on momentum_debug.py results:
# T=0.5 gives ~0 signals, T=0.3 gives ~1.5/month (matches original discovery!)
...
STRATEGIES = {
    'US100_M5': {
        ...
        'lb': 20, 'thresh': 0.3, 'sl': 1.5, 'rr': 2.0,  # T=0.3!
        'filter': 'atr',  # ATR 15-80
    ...
    'US30_M5': {
        ...
        'filter_file': None,
        'session': (time(15, 30), time(22, 0)),
        'lb': 20, 'thresh': 0.3, 'sl': 1.5, 'rr': 1.5,  # T=0.3, RR 1.5!
        'filter': 'rsi',
        'spread': 3.0,
        'expected_pf': 3.29,
    },
    'DAX40_M5': {
        'file': 'dax40_m5_data.csv',
        'filter_file': None,
        'session': (time(9, 0), time(17, 30)),
        'lb': 20, 'thresh': 0.3, 'sl': 1.5, 'rr': 2.0,  # T=0.3!
        'filter': 'rsi',
        'spread': 1.5,
        'expected_pf': 2.52,
    ...
    'US30_M1': {
        'file': 'us30_m1_data.csv',
        'filter_file': 'us30_m5_data.csv',  # M5 trend filter!
        'session': (time(15, 30), time(22, 0)),
        'lb': 10, 'thresh': 0.7, 'sl': 1.5, 'rr': 1.5,
        'filter': 'm5_trend',
        'spread': 3.0,
        'expected_pf': 2.33,
    },
...
class PortfolioBacktest:
    ...
    def _header(self):
        print()
        print("=" * 70)
        print("🎯 PORTFOLIO BACKTEST - PÔVODNÉ DISCOVERY STRATÉGIE")
        print("=" * 70)
        print(f"\n💰 Štartovací kapitál: €{self.initial:,.2f}")
        print()
        print("┌────────────┬────┬────────────┬──────┬──────┬───────┐")
        print("│ Market     │ TF │ Filter     │ PF   │ RR   │ T     │")
        print("├────────────┼────┼────────────┼──────┼──────┼───────┤")
        print("│ US100      │ M5 │ ATR 15-80  │ 3.26 │ 2.0  │ 0.3   │")
        print("│ SP500      │ M5 │ RSI < 70   │ 3.21 │ 2.0  │ 0.3   │")
        print("│ US30       │ M5 │ RSI < 70   │ 3.29 │ 1.5  │ 0.3   │")
        print("│ DAX40      │ M5 │ RSI < 70   │ 2.52 │ 2.0  │ 0.3   │")
        print("│ US30       │ M1 │ M5 trend   │ 2.33 │ 1.5  │ 0.7   │")
        print("└────────────┴────┴────────────┴──────┴──────┴───────┘")
        print()
        print("Params: LB=20 (M5), LB=10 (M1), SL=1.5×ATR")
    ...
    def _load(self):
        ...
        for name, cfg in STRATEGIES.items():
            ...
            # Load filter data for M1
            if cfg.get('filter_file'):
                df_filter = load_mt5_csv(cfg['filter_file'])
                if df_filter is not None:
                    df_filter = add_indicators(df_filter)
                    self.filter_data[name] = df_filter
    ...
    def _generate(self):
        ...
        for name, cfg in STRATEGIES.items():
            ...
            df = self.data[name]
            df_filter = self.filter_data.get(name)
            ...
            for i in range(50, len(df) - 100):
                ...
                # Momentum cross
                # Formula: momentum > ATR * thresh * lookback
                ...
                if filter_type == 'atr':
                    ...
                    # RSI filter (SP500, US30 M5, DAX40)
                    if pd.isna(bar['rsi']) or bar['rsi'] >= 70:
                        continue
                
                elif filter_type == 'm5_trend':
                    # M5 trend filter (US30 M1)
                    if pd.isna(bar['rsi']) or bar['rsi'] >= 70:
                        continue
                    
                    if df_filter is not None:
                        try:
                            m5_bar = df_filter.loc[:ts].iloc[-1]
                            if not m5_bar.get('uptrend', False):
                                continue
                            if pd.notna(m5_bar.get('rsi')) and m5_bar['rsi'] >= 70:
                                continue
                        except:
                            continue
    ...
    def _execute(self):
        ...
        for sig in self.signals:
            ...
            pnl_eur = self.balance * (pnl_pct / 100)
    ...
    def _results(self):
        ...
        print("-" * 70)
        print(f"   {'Stratégia':<12} {'Obch':>5} {'WR':>7} {'PF':>7} {'Expect':>7} {'Diff':>7} {'P&L':>12}")
        print("-" * 70)
        ...
        for name, cfg in STRATEGIES.items():
            ...
            if strat_trades:
                ...
                exp = cfg['expected_pf']
                diff = spf - exp
                status = "✅" if spf >= 1.5 else "⚠️" if spf >= 1.0 else "❌"
                
                print(f"   {status} {name:<10} {len(strat_trades):>5} {swr:>6.1f}% "
                      f"{spf:>6.2f} {exp:>6.2f} {diff:>+6.2f} €{spnl:>+10,.2f}")
        print()
        ...
        # Export
        pd.DataFrame(self.trades).to_csv('portfolio_results.csv', index=False)
        print(f"\n📁 Exportované: portfolio_results.csv")
